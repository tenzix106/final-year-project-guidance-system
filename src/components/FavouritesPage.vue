<template>
  <div class="max-w-6xl mx-auto py-8">
    ...
    <div v-else class="space-y-6">
      <div v-for="(favourite, index) in favouriteService.favourites.value" :key="favourite.id"
        ...
        <div class="border-t border-gray-200 pt-4 mb-4">
          ...
          <p v-else class="text-secondary-600 text-xs mt-2 flex items-center">
            <Check class="w-3 h-3 mr-1" /> {{ researchPapers.get(favourite.id).length }} papers retrieved • Click "View Papers" to review
          </p>
    ...
    <!-- Progress Tracking Confirmation Modal -->
    <div v-if="showProgressConfirm" 
         class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4"
         @click.self="cancelProgressTracking">
      <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden animate-slide-up">
        <!-- Modal Header -->
        <div class="bg-gradient-to-r from-primary-500 to-primary-600 text-white px-6 py-4">
          <h2 class="text-xl font-bold">Start Progress Tracking</h2>
        </div>
        ...
        <div class="p-6">
          <div class="flex items-start space-x-4 mb-6">
            ...
            <div>
              <h3 class="text-lg font-semibold text-gray-900 mb-2">Enable Timeline Tracking?</h3>
              <p class="text-gray-600 mb-3">
                This will create a project timeline for <span class="font-semibold">"{{ progressConfirmTitle }}"</span> with 5 phases:
              </p>
              ...
              <div class="text-sm text-gray-600 mt-3 space-y-1">
                ...
                <div class="flex items-center">
                  <Check class="w-4 h-4 mr-2 text-green-600" />
                  <span>Monitor your progress percentage</span>
                </div>
                <div class="flex items-center">
                  <Check class="w-4 h-4 mr-2 text-green-600" />
                  <span>Stay organized throughout your project</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="flex gap-3">
            <button 
              @click="cancelProgressTracking"
              class="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors font-medium">
              Cancel
            </button>
            <button 
              @click="confirmProgressTracking"
...
import { 
  ...
  Loader2, Save, TrendingUp, BookOpen, ExternalLink, FileDown, Check 
...
const progressConfirmFavouriteId = ref(null)
const progressConfirmTitle = ref('')
...
const startProgressTracking = (favouriteId) => {
  const favourite = favouriteService.favourites.value.find(f => f.id === favouriteId)
  progressConfirmFavouriteId.value = favouriteId
  progressConfirmTitle.value = favourite?.title || 'this project'
  showProgressConfirm.value = true
}

const cancelProgressTracking = () => {
  showProgressConfirm.value = false
  progressConfirmFavouriteId.value = null
  progressConfirmTitle.value = ''
}

const confirmProgressTracking = async () => {
  const favouriteId = progressConfirmFavouriteId.value
  showProgressConfirm.value = false
